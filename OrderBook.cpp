...
std::string OrderBook::getNextTime(std::string timestamp)
{
    ...
    for (OrderBookEntry& e : orders)
    {
        if (e.timestamp > timestamp) 
        {
            ...
            passedStep.push_back(timestamp);
            std::cout << "Passed " << passedStep.size() << " time steps" << std::endl;
            break;
        }
    }
    if (next_timestamp == "")
...
std::vector<std::string> OrderBook::passedTimestamp()
{
    return passedStep;
}
...
// bids = orderbook.bids
    ...
    //Check if max bid is lower than min ask --> if lower, no need to loop through the orders 
